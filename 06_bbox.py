...
pdf_path = f'data/pdf_sample/five.pdf'
...
for page in PDFPage.create_pages(doc):
    ...
    page_data['page_id'] = layout.pageid
    ...
    for obj in layout._objs:
        if isinstance(obj, pdfminer.layout.LTTextBoxHorizontal):
            ...
            # print(obj.bbox)
            data_item['text'] = obj.get_text().replace('\n', '')
...
for index, page in enumerate(pdf_data['page_list']):
    ...
    for text_item in page['text_list']:
        ...
        if text_item.get('text') != None:
            cv2.putText(image, text_item['text'], (int(text_item['bbox']['x1']), int(
                text_item['bbox']['y1'] + 10)), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.4, colors['black'], 1, cv2.LINE_AA)
